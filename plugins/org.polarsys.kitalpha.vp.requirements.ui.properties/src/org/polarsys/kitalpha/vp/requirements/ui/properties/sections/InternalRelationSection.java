 ...
 * @author Joao Barata
 */
public class InternalRelationSection extends AbstractSection {
  ...
  protected BasicReqIFElementGroup reqIFElementGroup;
	...
	public boolean select(Object eObject) {
		...
		if (KitalphaRequirementsUIPropertiesPlugin.isViewpointActive(eObjectToTest) && eObjectToTest instanceof InternalRelation) {
			return true;
		}
		return false;
	}
	...
	*/
	public void setInput(IWorkbenchPart part, ISelection selection) {
		...
		if (newEObject instanceof InternalRelation) {
			loadData(newEObject);
		}
	}
    ...
    sourceField = new SimpleSemanticField(getReferencesGroup(),
      Messages.getString("InternalRelation.SourceLabel"), getWidgetFactory(), new CurrentOrUpperBlockArchitectureFilteringController()); //$NON-NLS-1$
    sourceField.setDisplayedInWizard(displayedInWizard);

    targetField = new SimpleSemanticField(getReferencesGroup(),
      Messages.getString("InternalRelation.TargetLabel"), getWidgetFactory(), new CurrentOrUpperBlockArchitectureFilteringController()); //$NON-NLS-1$
    ...
    sourceField.loadData(capellaElement, RequirementsPackage.eINSTANCE.getInternalRelation_Source());
    targetField.loadData(capellaElement, RequirementsPackage.eINSTANCE.getInternalRelation_Target());
  }
